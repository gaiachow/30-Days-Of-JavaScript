...
![30 Days of JavaScript: Day 02](../images/banners/day_1_2.png)
...
2. Strings - any data inside single quotes, double quotes, or backticks
...
As a rule of thumb, we do not compare non-primitive data types.
...
Now, let us see how we can use the random() method to generate a random number between 0 and 10:
...
A string could be a single character, a paragraph, or a page.
If the string is too long to fit on one line, we can use the backslash character (`\`) at the end of each line to indicate that the string will continue on the next line.
...
1. *length*: The string *length* property returns the number of characters in a string, including whitespace.
...
9. *includes()*: Takes a substring and checks if it exists in the string.
...
16. *startsWith()*: Takes a substring as an argument and checks if the string starts with the specified substring.
    It returns a boolean (true or false).
...
This is not the regular expression section, so do not panic!
...
- Casting: Converting one data type to another data type.
  We use _parseInt()_, _parseFloat()_, _Number()_, the + operator, _String()_.
  When we do arithmetic operations on string numbers, they should be converted to an integer or a float first; otherwise, the operation returns `NaN` (Not-a-Number).

#### String to Int

We can convert string numbers to numbers.
Any number inside quotes is a string number.
...
1. 'Love is the best thing in this world. Some found their love and some are still looking for their love.' Count the number of times the word 'love' appears in this sentence.
